...
function generateRPCMethods(constructor, rpc, isPromiseBased) {
    ...
    for (var k in callSpec) {
        var spec = callSpec[k].split(' ');
        ...
        const fn = createRPCMethod(methodName, spec);
        constructor[k] = isPromiseBased ? promisify(fn) : fn;
        constructor[methodName] = constructor[k];
    }

}

function RpcClient(opts, isPromiseBased = false) {
    opts = opts || {};
    ...
    generateRPCMethods(this, rpc, isPromiseBased);
}
