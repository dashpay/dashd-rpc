...
function innerRpc(request, callback) {
  ...
  const req = this.protocol.request(options, (res) => {
    ...
    res.on('end', () => {
      ...
      if (res.statusCode === 500 && JSON.parse(buf.toString('utf8')).error.code === -8 && JSON.parse(buf.toString('utf8')).error.message === 'quorum not found') {
        const quorumNotFoundError = new Error(`Dash JSON-RPC: ${JSON.parse(buf.toString('utf8')).error.message}`);
        quorumNotFoundError.code = -8;
        callback(quorumNotFoundError);
        return;
      }
      if (res.statusCode === 500 && JSON.parse(buf.toString('utf8')).error.code === -32603 && JSON.parse(buf.toString('utf8')).error.message === 'This is not a masternode') {
        const noMasternodeError = new Error(`Dash JSON-RPC: ${JSON.parse(buf.toString('utf8')).error.message}`);
        noMasternodeError.code = -32603;
        callback(noMasternodeError);
        return;
      }
      let parsedBuf;
      try {
