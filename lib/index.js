...
RpcClient.config = {
    ...
    sensitive: false//allow sensitive rpc call when at true
};
...
//Unless having set opts sensitive at true, these won't execute.
RpcClient.sensitiveCalls = [
    'clearbanned',
    'debug',
    'dumpprivkey', 
    'getnettotals',
...
function generateRPCMethods(constructor, apiCalls, rpc) {
    function createRPCMethod(methodName, argMap) {
        return function (cb) {
            //Check if we are allowed to deliver that rpc command
            if (RpcClient.sensitiveCalls.indexOf(methodName) > -1 && this.sensitive !== true) {
                return cb(new Error("Dash JSON-RPC: Trying to execute sensitive call without permission"));
            }
