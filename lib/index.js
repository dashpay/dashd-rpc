...
function rpc(request, path, callback) {
  const self = this;
  const task = function (taskCallback) {
    ...
    innerRpc.call(self, request, path, newCallback);
  };

  this.queue.push(task);
}
function innerRpc(request, path, callback) {
  const self = this;
...
RpcClient.prototype.batch = function (batchCallback, path, resultCallback) {
  this.batchedCalls = [];
  batchCallback();
  rpc.call(this, this.batchedCalls, path, resultCallback);
  this.batchedCalls = null;
};
...
RpcClient.prototype.setWallet = function (wallet) {
  this.path = '/wallet/'+ wallet;

  return this;
}

RpcClient.walletcallspec = {
  abandonTransaction: 'str',
  addMultiSigAddress: 'int str str',
  backupWallet: 'str', // check me
  dumpPrivKey: 'str',
  dumpWallet: 'str',
  encryptWallet: 'str',
...
function getRPCMethods(constructor, path, apiCalls, rpc) {
  function createRPCMethod(methodName, methodPath, argMap) {
    return function () {
  ...
  for (const k in apiCalls) {
    ...
    methods[k] = createRPCMethod(methodName, path, spec);
    methods[methodName] = constructor.prototype[k];
  }

  return methods;
}

function getWalletRPCMethods(constructor, apiCalls, rpc) {
  return {
    getWallet: function (walletName) {
      const path = '/wallet' + walletName;

      return getRPCMethods(constructor, path, apiCalls, rpc);
    }
  };
}
...
function generateRPCMethods(constructor, rpc) {
  const rpcMethods = getRPCMethods(constructor, '/', constructor.callspec, rpc);
  const walletMethods = getWalletRPCMethods(constructor, constructor.walletcallspec, rpc);
  ...
  for (const [method, func] of  Object.entries(allMethods)) {
    constructor.prototype[method] = func;
  }

  RpcClient.prototype.apiCalls = RpcClient.callspec;
  RpcClient.prototype.wallet = {
    apiCalls: RpcClient.walletcallspec,
  };
}

generateRPCMethods(RpcClient, rpc);
